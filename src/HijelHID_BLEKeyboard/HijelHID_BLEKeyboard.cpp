...
void HijelHID_BLEKeyboard::begin() {
    ...
    // 0x00: Swift Pair Sub Scenario (LE only pairing)
    // 0x80: Reserved RSSI
    std::string swiftPairData;
    swiftPairData += (char)0x06;
    swiftPairData += (char)0x00;
    swiftPairData += (char)0x03;
    swiftPairData += (char)0x00;
    swiftPairData += (char)0x80;
    swiftPairData += _deviceName; // Include the display name in the payload!
    pAdv->setManufacturerData(swiftPairData);
    
    // We intentionally omit adding service UUIDs directly to pAdv here, 
    // because with the Swift Pair name included, the primary packet exceeds 31 bytes.
    // Instead, we put the Service UUIDs in the Scan Response.
    ...
    // Bluetooth settings on all hosts (particularly Android and Windows).
    NimBLEAdvertisementData scanResponse;
    scanResponse.setName(_deviceName.c_str());
    
    // Add essential service UUIDs to scan response for Android/Windows compatibility
    scanResponse.addServiceUUID(NimBLEUUID((uint16_t)0x1812)); // HID Service
    scanResponse.addServiceUUID(NimBLEUUID((uint16_t)0x180F)); // Battery Service
...
void HijelHID_BLEKeyboard::_onConnect(NimBLEConnInfo& connInfo) {
    _connected  = true;
    _connHandle = connInfo.getConnHandle();
    _connState  = _ConnState::Connecting;
    _logNf("Host connected (handle=0x%04X).", _connHandle);
    Serial.printf("[%02lu:%02lu:%02lu] [SYSTEM] Bluetooth Connected (MAC: ", (millis()/1000)/3600, ((millis()/1000)%3600)/60, (millis()/1000)%60);
...
void HijelHID_BLEKeyboard::_onDisconnect() {
    ...
    if (_state == _BLEState::Running) {
        ...
        Serial.printf("[%02lu:%02lu:%02lu] [SYSTEM] Bluetooth Disconnected\n", (millis()/1000)/3600, ((millis()/1000)%3600)/60, (millis()/1000)%60);
        NimBLEDevice::startAdvertising();
    } else {
